...
/**
 * Service class để xử lý retrain model cũ
 * Flow: Chọn model cũ -> Load datasets của model -> Thêm datasets mới (optional) -> Config hyperparameters -> Retrain -> Ghi đè model cũ
 */
class ModelRetrainService {
  /**
   * Lấy thông tin model cũ và datasets đã sử dụng
   * @param {number} modelId - ID của model cần retrain
   * @returns {Promise<Object>} Model info với datasets
   */
    ...
    try {
      ...
      if (model.tblDatasetId) {
        ...
        if (dataset) {
          ...
          // Lấy danh sách email IDs từ dataset
          existingEmailIds = dataset.emails ? dataset.emails.map(e => e.id) : [];
        }
      }
  ...
  /**
   * Chuẩn bị training data từ sample IDs
   * @param {Array<number>} sampleIds - Array of email IDs
   * @returns {Promise<Array>} Formatted training samples
   */
  ...
  async startRetraining(userId, config) {
    try {
      ...
      const trainingRequest = {
        ...
        modelType: modelType,
        modelPath: model.path, // Truyền path của model cũ để retrain
        samples: samples,
        hyperparameters: {
      ...
      return {
        success: true,
        jobId: job.id,
        modelId: config.modelId, // Trả về modelId để biết model nào đang được retrain
